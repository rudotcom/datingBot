...
def read_person_encodings(enc_path=enc_path):
    names = []  # Список известных людей
    encodings = []  # Список кодов лиц

    # Просмотреть все файлы кодов
    for enc_name in os.listdir(enc_path):
        encoding_file = os.path.join(enc_path, enc_name)
        encoding = np.loadtxt(encoding_file)
        names.append(enc_name)
        encodings.append(encoding)

    return names, encodings


def write_person_encoding(name, encoding, enc_path=enc_path):
...
if __name__ == '__main__':
    known_face_names, known_face_encodings = read_person_encodings()
